<template>
    <div class="page">
      <div class="page__header">
        ...
        <div class="page__create" @click="addSensor()">
          <addSvg />
          <span>افزودن گیت‌وی </span>
        </div>
      </div>
      <div class="page__items">
        <template v-if="getLoading"> در حال بارگیری ... </template>
        <template v-else>
          <template v-if="gateways.length">
            <div class="page__item" v-for="(item, index) in gateways" :key="index">
          ...
          <template v-else> گیت‌ویی یافت نشد </template>
        </template>
      </div>
      ...
      <base-modal
        ...
        @close="dialog = false"
        :maxWidth="700"
        :title="`${mode === 'add' ? 'افزودن' : 'ویرایش'} گیت‌وی`"
      >
        <div>
      ...
      <confirmModal
        ...
        title="حذف گیت‌وی"
        message="آیا می خواهید گیت‌وی را حذف کنید"
  ...
  import server1Icon from "@/assets/svg/ic_server2.svg";
  ...
  export default {
    ...
    methods: {
      async getAll() {
        ...
        try {
          const { data } = await gatewayServices.getAll();
          this.gateways = data;
        } catch {
      ...
      async add() {
        ...
        try {
          if (this.mode === "add") {
            await gatewayServices.create(this.model);
          } else {
            await gatewayServices.edit(this.selected._id, {
      ...
      async deleteSensorConfirm() {
        try {
          await gatewayServices.delete(this.selected._id)
          await this.getAll();
        }
        catch {
          this.$toast.error("خطایی رخ داد");
